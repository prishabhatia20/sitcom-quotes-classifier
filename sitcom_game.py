...
class GameModel:
    ...
    def __init__(self):
        """
        Initializes the class attributes for managing the quiz game or model interaction.

        Attributes:
            score: an int tracking the user's current score in the game or quiz
            questions_shown: an int tracking number of quotes shown to the user so far
            active: a Boolean indicating whether the game is currently active
            data: a Pandas dataframe which is the dataset containing quotes and characters
            current_quote: a String representing quote currently being shown to the user
            current_answer: a String representing correct answer for the current quote
            characters: a list of all characters in the dataset
            quote_indices: a list of indices representing the row in the dataset to pull the quote from
            model_answer: a String representing the answer predicted by the model for the current quote
            correct_answer: a Boolean indicating whether the user's answer was correct or not
            model_correct_answer: a Boolean indicating if the model's answer for the current quote is correct
            model_score: an int tracking the model's score
            classifier: an instance of the Naive Bayes trained classification model
            vectorizer: the CountVectorizer used for transforming text data
            predictions: a list of predictions generated by the model for the current game
            user_answer: a String representing the user's answer
            characters_dict: a dict of characters mapped to the position they are rendered in (for answer validation)
"""
    ...
    def set_classifier(self, classifier, vectorizer):
        """
        Set the classifier and vectorizer

        Args:
            classifier: an instance of the Naive Bayes classifier trained for the user-selected show
            vectorizer: the CountVectorizer used to vectorize data
        """
    ...
    def pick_quotes(self):
        ...
        # Make sure there are 7 quotes and make sure they don't repeat
    ...
    def get_quote(self):
        ...
        # Locate the row of data specified by quote_indices
        row = self.data.iloc[self.quote_indices[self.questions_shown]]

        # Pull out the character and the quote
        character = row['character']
        quote = row['quote']

        # Set the current quote and current answer to character and quote
        self.current_quote = quote
        self.current_answer = character.lower()

        # Increase questions_shown
    ...
    def get_model_results(self):
        ...
        # Get the current quote make sure it is in a list
    ...
    def check_answer(self, x, y):
        ...
        # If the user is correct, update the score and correct_answer
        if self.user_answer == self.current_answer:
            self.correct_answer = True
            self.update_score()
        # If they are incorrect, update correct_answer
        else:
            self.correct_answer = False


        # If the model is correct, update the score and correct_answer
        if self.model_answer == self.current_answer:
            self.update_model_score()
            self.model_correct_answer = True
        # If it is incorrect, update correct_answer
        else:
...
class GameController:
    ...
    def handle_answer_click(self):
        ...
        # While the user has not clicked, wait for quit event or mouse press event
        while waiting_for_click:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # Pass the position to check_answer function
